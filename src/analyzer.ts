...
export class Analyzer {
  ...
  async #upgradePlaceholder(
    ...
    const server = this.#processServer(
      placeholder,
      packageJson,
      syntaxInfo,
      command
  ...
  #processServer(
    ...
    const node = findNodeAtLocation(syntaxInfo.wireitConfigNode, ['server']) as
      | undefined
      | JsonAstNode<true | false>;
    if (node == null) {
      return defaultValue;
    }
    if (node.value !== true && node.value !== false) {
      placeholder.failures.push({
        ...
        diagnostic: {
          severity: 'error',
          message: `The "server" property must be either true or false.`,
          location: {
            file: packageJson.jsonFile,
    ...
    if (value === true && command == null) {
      placeholder.failures.push({
        ...
        diagnostic: {
          severity: 'error',
          message: `A "server" script must have a "command".`,
          location: {
            file: packageJson.jsonFile,
            range: {
              length: node.length,
